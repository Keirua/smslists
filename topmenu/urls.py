...
urlpatterns = [
	...
	# url(r'^listings/(?P<category>\w+)/(?page=P<page>[0-9]+)/', views.Listings.as_view(), name='listings'),
	...
	url(r'^listing/(?P<category>\w+)/(?P<listing_id>\d+)/', views.ListingDetail.as_view(), name='listing_detail'),
	...
	url(r'^search_results/(?P<category>\w+)/', views.SearchResults.as_view(), name='search_results'),
