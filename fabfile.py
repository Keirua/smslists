...
CONFIG = {
    ...
    'staging': {
        'hosts': ['root@'],
        'dest': '/usr/local/lib/smslists/',
        'env_name': 'staging',
        'requirements': 'deployment',
        'static': '/var/www/smslists'
    },
    'live': {
        'hosts': ['root@api.worlddev.io'],
        'dest': '/usr/local/lib/smslists/',
        'env_name': 'deployment',
        'requirements': 'deployment',
        'static': '/var/www/smslists'
...
def build(patch=False):
    ...
    app_files = formic.FileSet(
        include=[
            '/requirements/**', '/smscl/**', '/static/**', '/templates/**', '/manage.py', 'topmenu/**'
        ], exclude=[
            '/smscl/settings/environment.py', '*.pyc'
        ]
...
def deploy():
    ...
    with cd(env.dest):
        ...
        with cd(timestamp):
            remote_archive = '/tmp/smslists-%s-%s.tar.gz' % (timestamp, version)
            ...
            with hide('stdout'):
                ...
                run('env/bin/python manage.py collectstatic -v 0 -l --noinput -c')
